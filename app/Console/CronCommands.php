...
final class CronCommands
{
    public static function registerAll(): void
    {
        ...
        // Mobile Builder GitHub/Worker build senkronizasyonu — her 2 dakikada bir
        CronScheduler::register('mobile:build-sync', function () {
            try {
                $r = \App\Modules\Builder\Services\MobileBuildSyncService::run();
                return sprintf('jobs=%d done=%d failed=%d', $r['jobs'], $r['done'], $r['failed']);
            } catch (\Throwable $e) {
                return 'error: ' . $e->getMessage();
            }
        });

        // Mobile build artifact temizleme - günlük
        CronScheduler::register('mobile:artifact-clean', function () {
            $dir = AHO_ROOT . '/storage/mobile-builds'; $deleted = 0;
            if (is_dir($dir)) foreach (glob($dir . '/*/*') ?: [] as $file) {
                if (is_file($file) && (time() - (filemtime($file) ?: time())) > 30 * 86400) { @unlink($file); $deleted++; }
            }
