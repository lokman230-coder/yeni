    ...
    public static function processJob(int $jobId): array
    {
        ...
        try {
            switch ($job['export_type']) {
                ...
                case 'pwa':
                    $outputPath = "$exportDir/$fileName.zip";
                    self::generatePwa($project, $pages, $outputPath);
                    break;
...
export default function App() {
    ...
    private static function generatePwa(array $project, array $pages, string $outputPath): void
    {
        $tmp = sys_get_temp_dir() . '/ahost-pwa-' . bin2hex(random_bytes(6));
        @mkdir($tmp, 0775, true);
        $html = ExportService::siteToHtml($project, $pages);
        $html .= '<script>if("serviceWorker" in navigator){window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js"));}</script>';
        file_put_contents($tmp . '/index.html', $html);
        file_put_contents($tmp . '/manifest.json', json_encode(['name'=>$project['name'] ?? 'Ahost Mobile App','short_name'=>$project['name'] ?? 'Ahost App','start_url'=>'./','display'=>'standalone','background_color'=>'#ffffff','theme_color'=>'#0ea5e9','icons'=>[]], JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE));
        file_put_contents($tmp . '/sw.js', "self.addEventListener('install',e=>self.skipWaiting());self.addEventListener('activate',e=>self.clients.claim());self.addEventListener('fetch',e=>e.respondWith(fetch(e.request).catch(()=>caches.match(e.request))));");
        $zip = new \ZipArchive();
        if ($zip->open($outputPath, \ZipArchive::CREATE|\ZipArchive::OVERWRITE) !== true) throw new \RuntimeException('PWA ZIP oluşturulamadı');
        foreach (['index.html','manifest.json','sw.js'] as $file) $zip->addFile($tmp.'/'.$file,$file);
        $zip->close();
