    ...
    <div class="aho-card" style="padding:0;overflow:auto">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:13px">
            ...
            <?php foreach ($jobs as $j):
                $processed = (int)$j['imported'] + (int)$j['skipped'] + (int)$j['errors'];
                $incomplete = (int)$j['total'] > 0 && $processed < (int)$j['total'];
                $pct = (int)$j['total'] > 0 ? min(100, (int)round($processed / (int)$j['total'] * 100)) : 0;
                ...
                if ($j['status'] === 'completed' && $incomplete) {
                    $badge = ['Eksik', '#b45309', '#fef3c7'];
                }
            ?>
                <tr style="border-top:1px solid var(--aho-color-border)">
                    ...
                    <td style="padding:10px 16px;text-align:right;min-width:180px">
                        <div style="font-size:12px;color:var(--aho-color-ink-600)"><?= $processed ?> / <?= (int)$j['total'] ?></div>
                        <div style="background:#e5e7eb;border-radius:6px;height:6px;overflow:hidden;margin-top:2px">
                            <div style="width:<?= $pct ?>%;height:100%;background:<?= ($j['status']==='completed' && !$incomplete)?'#059669':'#0891b2' ?>"></div>
                        </div>
                    ...
                    <td style="padding:10px 16px;text-align:right;white-space:nowrap">
                        <?php if (($j['status'] !== 'completed' && $j['status'] !== 'failed') || (int)$j['errors'] > 0 || $incomplete): ?>
