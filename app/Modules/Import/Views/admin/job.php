...
$processed = (int)$job['imported'] + (int)$job['skipped'] + (int)$job['errors'];
$incomplete = (int)$job['total'] > 0 && $processed < (int)$job['total'];
$pct = (int)$job['total'] > 0 ? min(100, (int)round($processed / (int)$job['total'] * 100)) : 0;
?>
<div class="aho-admin-page">
    ...
    <div class="aho-card" style="padding:20px;margin-bottom:16px">
        <div style="display:flex;justify-content:space-between;margin-bottom:8px">
            ...
            <span style="color:var(--aho-color-ink-500);font-size:13px">Durum: <?= e($job['status'] === 'completed' && $incomplete ? 'incomplete' : $job['status']) ?></span>
        </div>
        <div style="background:#e5e7eb;border-radius:8px;height:12px;overflow:hidden">
            <div style="width:<?= $pct ?>%;height:100%;background:<?= ($job['status']==='completed' && !$incomplete)?'#059669':'#0ea5e9' ?>;transition:width .5s"></div>
        </div>
        <?php if ($job['status'] !== 'completed' || (int)$job['errors'] > 0 || $incomplete): ?>
