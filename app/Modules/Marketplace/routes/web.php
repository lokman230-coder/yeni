...
$router->group(['middleware' => ['locale', 'currency']], function (Router $router) {
    ...
    $router->get('/marketplace/satin-aldiklarim', [MarketplaceController::class, 'purchases'])->middleware(['customer.auth'])->name('marketplace.purchases');
    $router->post('/marketplace/satin-aldiklarim/{id}/token', [MarketplaceController::class, 'token'])->middleware(['csrf', 'customer.auth']);
    $router->get('/marketplace/download/{token}', [MarketplaceController::class, 'download'])->name('marketplace.download');
