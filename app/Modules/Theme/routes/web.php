...
use App\Core\Router;
use App\Modules\Theme\Controllers\AdminThemeController;
...
$router->group(['prefix' => 'admin', 'middleware' => ['locale', 'admin.auth']], function (Router $router) {
    $router->get('/temalar', [AdminThemeController::class, 'index'])->name('admin.themes');
    $router->post('/temalar/aktif-et', [AdminThemeController::class, 'activate'])->middleware(['csrf']);
});
